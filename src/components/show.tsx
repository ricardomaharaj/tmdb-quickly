        ...
        <div className='flex flex-col gap-2'>
          ...
          {sp.tab === 'Seasons' && (
            <>
              <Grid variant='123'>
                {show?.seasons?.map((x) => (
                  <Link to={`season/${x.season_number}`} key={x.id}>
                    <Card
                      img={x.poster_path}
                      pri={x.name}
                      sec={
                ...
                <Link to={`/person/${x.id}`} key={x.id}>
                  <Card
                    ...
                    ter={x.roles
                      ...
                      ?.map(
                        (x) =>
                          `${rmVoiceTag(x.character) ?? 'Unknown'} ${numGt0(x.episode_count) ? `(${x.episode_count} Eps)` : ''}`,
                      )
                      ?.join(' | ')}
                  />
                </Link>
                ...
                <Link to={`/person/${x.id}`} key={x.id}>
                  <Card
                    ...
                    ter={x.jobs
                      ...
                      ?.map(
                        (x) =>
                          `${x.job} ${numGt0(x.episode_count) ? `(${x.episode_count} Eps)` : ''}`,
                      )
                      ?.join(' | ')}
                  />
                </Link>
